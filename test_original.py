...
def run(#train에서 epoch 끝날때마다 시행
    ...
    if training:  # called by train.py train에서 부르는거?
        ...
        #gating_network = MyModelWithConditionalDownscaling().to(device).half() if half else MyModelWithConditionalDownscaling().to(device).float() #syh edit dfm 로드
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar): #시작
        ...
        with dt[0]: #데이터 전처리(pre-process)
            ...
            im_jpeg = F.interpolate(im, size=(512,512), mode='bilinear', align_corners=False)

            resized_imgs_to_ycbcr = TJ_ycbcr.to_ycbcr(im_jpeg, 1.0)
            ...
            nb, _, height, width =  dequantized_dct.shape  # batch size, channels, height, width 
    ...
    with open(f"{save_dir}/mAP_results.txt", "w") as file:
        file.write(f"mAP@0.50: {map50}\n")
        file.write(f"mAP@0.50-0.95: {map}\n")
        for i, c in enumerate(ap_class):
            file.write(f"Class {names[c]} - AP: {ap[i]}\n")
    ...
    #print("validation average psnr : ", sum(total_psnr) / len(total_psnr))    
    ...
    #return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist(), bpp_loss_val.cpu() / len(dataloader), #df_val.cpu()/ len(dataloader),  total_psnr/ len(dataloader)), maps, t

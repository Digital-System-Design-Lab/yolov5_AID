...
class Loggers:
    ...
    def __init__(self, save_dir=None, weights=None, opt=None, hyp=None, logger=None, include=LOGGERS):
        ...
        if clearml and "clearml" in self.include:
            try:
                ...
                LOGGER.warning(
                    f"{prefix}WARNING ClearML is installed but not configured, skipping ClearML logging."
    ...
    def on_fit_epoch_end(self, vals, epoch, best_fitness, fi):  # val에도 bpp_loss 뜰 수 있게
    # Callback runs at the end of each fit (train+val) epoch
        x = dict(zip(self.keys, vals))  # keys와 vals를 결합하여 사전을 만듭니다.
        if self.csv:
            file = self.save_dir / "results.csv"
            n = len(self.keys) + 1  # 열의 수 (epoch 포함)
            ...
            with open(file, "a") as f:
                # 포맷 문자열 생성
                format_str = ("%20.5g," * n ).rstrip(",")
                
                # 디버깅 출력
                # print(f"Expected format string: '{format_str}'")
                # print(f"Expected values: {tuple([epoch] + vals)}")
                
                # # 포맷 문자열의 길이와 값의 개수 확인
                # if format_str.count('%20.5g') != len([epoch] + vals):
                #     raise ValueError(f"Format string length {format_str.count('%20.5g')} does not match the number of values {len([epoch] + vals)}")
                
                # 값을 포맷팅하여 파일에 작성
                f.write(header + (format_str % tuple([epoch] + vals)) + "\n")
    ...
    #         #    
    #         header_format = "{:20}," * len(["epoch"] + self.keys)
    #         header = header_format.format(*(["epoch"] + self.keys)).rstrip(",") + "\n"
    #         s = "" if file.exists() else header
    #         #
